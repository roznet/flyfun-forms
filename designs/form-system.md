...
- **Flattening is ours, not pypdf's:** the app always asks for `flatten=true`. pypdf 4 can't flatten — its `auto_regenerate` only sets `/NeedAppearances`, which leaves every field editable and tells the viewer to redraw them, discarding the fitted appearances above. `fillers/_flatten.py` paints each widget's appearance stream into the page and removes the widgets and `/AcroForm`; output renders pixel-identical to the unflattened fill. XObject names are numbered across the document because pages can share one `/Resources` dictionary.

...
address), full document editor, scan fills empty person fields — `7061e29`.

## 2. How to work

- **Order is 1 → 2 → 3.** PR 1 and PR 2 both reshape the flight editor;
  running them in parallel means constant conflicts.
- **Branches.** PR 1 continues on `android-app` (it already carries `7061e29` and
  this doc). PR 2 and PR 3 each branch from `main` once the previous one has merged.
